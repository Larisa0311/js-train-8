...
function createArray(...elements) {
  ...
  const createdArr = Array.of(...elements);
  return (createdArr.at(0) !== undefined) ? createdArr : "Error";
}
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr) === true) {
    ...
    let indexLast = arr.lastIndexOf(element);
  if (index !== -1) {arrIndex[0] = index};
  if (indexLast !== -1 && index != indexLast) {
    arrIndex[1] = indexLast;  }
    return arrIndex;
  } else {return arr = []};
}
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if ( arr.indexOf(target) !== -1 || arr.indexOf(start) !== -1 || arr.lastIndexOf(end) !== -1)
...
function sortByKey(arr, key) {
  ...
  // якщо a[key] > b[key] повертаємо 1            (-1) - error
  // інакше повертаємо 0
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if (Array.isArray(arr) === true ) {
    if (arr.indexOf(start) !== -1 || arr.lastIndexOf(end) !== -1) {
...
function customShift(arr) {
  ...
  if (Array.isArray(arr) === true && arr.at(0) !== undefined ) {let shiftedElement = arr.shift();
...
function customUnshift(arr, ...elements) {
  ...
  } else {return "Нема вхідного масиву";}
}
...
function customSome(arr, condition) {
  ...
  // Повертаємо результат перевірки
  if (Array.isArray(arr) === true && condition) {
    return arr.some(condition, arr);
  } else {return false;}
}
...
function customAt(arr, index) {
  ...
  if (Array.isArray(arr) === true && arr.at(index) !== undefined ) {
    let elemArr = arr.at(index);
    return elemArr, (typeof elemArr === "string") ? `Елемент є рядком. ${elemArr}`  :
           ((typeof elemArr === "number") ? `Елемент є числом. ${elemArr}` :
           `Елемент є обєктом. ${elemArr}`);
...
function customIncludes(arr, element) {
  ...
  if (Array.isArray(arr) === true && arr.includes(element) === true) {
    const includArr = arr.filter((value, index, arr) => value === element);
    return includArr, includArr.length;
  } else {return false;}
}
